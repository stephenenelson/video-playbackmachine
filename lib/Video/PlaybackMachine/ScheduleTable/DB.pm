...
sub get_offset {
    ...
    my $sth =
      $self->getDbh()
      ->prepare(
        "select EXTRACT(EPOCH FROM CURRENT_TIMESTAMP - TIMESTAMPTZ '$date')");
    $sth->execute()
      or return;
...
sub get_offset_to_first {
    ...
    my $sth =
      $self->getDbh()
      ->prepare(
'SELECT EXTRACT( EPOCH from CURRENT_TIMESTAMP - start_time ) FROM schedule_times ORDER BY start_time LIMIT 1'
      );
    $sth->execute()
      or return;
    my ($offset) = $sth->fetchrow_array()
...
sub _entry_for {
    ...
    return Video::PlaybackMachine::ScheduleEntry->new( int($start_time),
        $movie );
}
